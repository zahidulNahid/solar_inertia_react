...
export function Sidebar({ lists }) {
  ...
  const NavigationContent = useMemo(() => {
    return (
      <nav className="min-w-[272px] flex-1 px-10 backdrop-blur-lg text-black">
        {lists.map((item) => {
    ...
    return (
      <div className="fixed left-0 right-0 top-0 z-50 flex h-16 items-center justify-between bg-[#36007B] p-0 md:hidden">
        <Link href="/dashboard" className="flex items-center gap-2">
          ...
          {/* <img src="/assets/img/logo.svg" alt="psykick.club" width="194" height="38" /> */}
        </Link>
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          ...
          <SheetContent side="left" className="w-[300px] bg-[#36007B] p-0 text-white">
            <div className="flex h-full flex-col">
              <div className="p-6">
                <Link href="/" className="flex items-center gap-2">
                  {/* <img src="/assets/img/logo.svg" alt="psykick.club" width="120" height="60" /> */}
                </Link>
              </div>
              {NavigationContent}
              {UserProfile}
